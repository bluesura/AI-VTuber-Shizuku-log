...
5. **handoffはソースタイトルを保持する。** wikiの出典表記が動画タイトルを使うため、s5はYouTube配信のタイトル(meta.json)・X投稿の本文冒頭を各ブロックに出力する。この付与を外さない。
6. **5つの人間ゲートを迂回しない。** ①逐語(VERIFY) ②公開(ADOPT→wiki貼付) ③辞書・スキーマ変更 ④ループ確定(CLOSE) ⑤月次棚卸し。スクリプトはゲートの手前で止まり、判断を人間に残す。
