...
**2b. Claudeチャットで抽出 — 1配信＝1チャットが正しいやり方です**

（同じ配信が `_partN` に分かれている場合だけは、その配信の全partを**同じ1つのチャット**で順番に処理します。別配信は必ず別チャット。詳細は下の「分割パック（part）の渡し方」。）
...
1. **新しいClaudeチャット**を開く（スキル不要。パックに指示が全部入っています）。どのモデル・思考設定を使うかは「4. LLMモデルの選び方」を参照。
   ...
   **分割パック（part）の渡し方 — 重要**
   `_part1` `_part2` … に分かれているのは、配信の中身が別だからではなく、**1本が長すぎて1チャットに収まらないので機械的に切っただけ**です。part同士は同じ配信の前半・後半で、時系列が地続きです。だから:
   - ✅ **同じチャットで、1つずつ順番に渡す。** まず part1 を貼って抽出→JSONLを受け取り、**同じチャットのまま** part2 を貼って「続きです。前半の文脈を踏まえて抽出して」と送る。こうすると前半の内容が履歴に残り、掛け合いの片側だけが別partにある名言や、前半で開いた伏線が後半で回収される流れを、モデルが繋げられます。
   - ❌ **全partを1メッセージにまとめて貼らない。** 分割した意味が消え、元の入りきらないサイズに戻ります（入っても中盤の取りこぼしが増える）。
   - ❌ **partごとに別チャットに分けない。** 後半チャットから前半の文脈が見えず、判断が分断されます。
   - ⛔ **別配信を同じチャットに混ぜない。** part分割（同一配信）は同じチャット、別配信は別チャット。混ぜると動画ID・秒数が混ざったカードが出て試聴リンクがずれます。
   - 保存は **partごとに別ファイル**（`extract_..._part1.jsonl` / `_part2.jsonl`）でよい。取込先(`--from`)が同じなので `s2_batch --apply` がまとめて取り込み、WORKLISTの両行にチェックが付きます。
   - partが5つも6つもできる配信は、1本が極端に長いということ。`--max-chars` をむしろ**上げて**part数を減らすとチャットの往復が減って楽になることがあります（「同じチャットで順番に」の原則は変わりません）。
2. `data/packs/extract_*.txt` を**添付**し、「このファイルの指示に従ってカードをjsonlファイルで抽出して」と送る。
...
```
python scripts/s3_match.py --status       # まず現状: 未回収ループ数・カード数・月別分布
python scripts/s3_match.py                # 照合パック生成（1回最大1500ペアで頭打ち）
...
**大量に溜まっている場合は、区切って回します（重要）。**
過去ログをまとめて抽出すると、未回収ループが数百〜数千になり、1回の照合ペアが万単位に膨れ得ます。
これを一度にLLMへ判定させてはいけません（コンテキストに入らず、判定も雑になります）。次の2つで小さく刻みます。

- **1パックの上限**: 既定で**1回1500ペア**まで。上限に達したら「続きは次回」と表示されます。
  判定→取込→もう一度 `s3_match` を実行、を繰り返せば、判定済みペアは記録されているので**続きだけ**が出ます。
- **期間で区切る**: `--until 2026-03-31` のように指定すると、**その日までに開いたループだけ**を照合します。
  古い月から順に `--until` を進める（例: 2月末→3月末→…）と、台帳を育てながら少しずつ閉じられます。

```
# 例: 大量データを月末ごとに区切って進める
python scripts/s3_match.py --until 2026-02-28      # 2月末までのループを照合
#   → 判定 → --ingest → s4_packet でCLOSE確認 →
python scripts/s3_match.py --until 2026-03-31      # 次は3月末まで
```

`--max-pairs 500` で1回の量をさらに減らせます。**判定の母数を小さく保つのが、質と速度の両方に効きます。**

**一度大きく回してしまった後の続け方（やり直し）**
巨大な照合を一度実行すると、その全ペアが「照合済み」として記録され、次回から
`照合対象なし（… 新規ペアなし）` となって進めなくなります。この記録だけを安全に消してやり直せます。

```
python scripts/s3_reset.py            # 現状表示（変更なし）
python scripts/s3_reset.py --apply    # 照合済み記録(match_seen)をクリア。.bak を残す
```

これは**「どのペアを短リストに出したか」の記録だけ**を消します。カード・台帳・確定済みのクローズ提案には触れません。
クリア後は `--until` で古い月から区切って出し直してください。
なお前回生成された巨大な `data/packs/match_*.txt` は使わないので削除して構いません（まだ何も確定していません）。

計算量は常に「新規×open」だけ。全履歴を舐める工程は存在しません。前絞りは、ありふれた語（「しずく」「配信」等のストップワード）を除いた**3文字以上の共通語**で行うので、無関係な組み合わせは最初から除外されます。
...
```
python scripts/s4_packet.py                       # 既定: 名言候補は「30日熟成」後に登場
python scripts/s4_packet.py --mature-days 0       # 即時に全部見たいとき
```

**大量候補は区切ってレビューします（重要）。** 2年分を一気に抽出すると、1枚のパケットに年表数百・名言数百が並び、レビュー不能になります。次で小さく分けます。

```
python scripts/s4_packet.py --until 2026-02-28 --kinds event      # 2月末までの年表だけ
python scripts/s4_packet.py --kinds event --limit 50              # 年表を50件だけ
python scripts/s4_packet.py --since 2026-03-01 --until 2026-03-31 # 3月分だけ
```

- `--until` / `--since` … 候補の日付で期間を区切る。
- `--kinds` … `event,capability,quote,profile` から種別を限定（`quote`=名言, `profile`=紹介ページ供給）。**年表→機能→名言の順**で片付けると楽です。
- `--limit N` … 各セクションN件まで。
- **未処理の候補は、パケットを何度作り直しても出続けます**（`s5_apply` で採用/却下/確定した分だけ次回から外れます）。だから区切りを変えて何度でも作り直して構いません。
- `--reset` は、`s5_apply` で処理済みにしたカードを**もう一度レビュー対象に戻したい**ときだけ使います（採用済み=approved等のstatusは戻りません。あくまで「処理済みマーク」を外すだけ）。通常は不要です。

**補足を書き足せます。** 各カードの `補足:` 行に書いた内容は、ADOPT採用時に **handoff へ引き継がれ**、wiki起草チャット（③）の材料になります。試聴して直すのは `逐語:` 行、文脈やニュアンスのメモは `補足:` 行、と使い分けます。
...
**種別ごとに、途中まででも適用してよい（大量にあるときの推奨）。**
候補が数百件あるときは、全部を一度にチェックする必要はありません。例えば**年表だけ全部チェックして、名言・機能・供給は空欄のまま** `s5_apply` を実行して構いません。理由:
- `s5_apply` は `[x]` の付いた行だけを処理し、`[ ]` のままの行は**完全に無視**します。だから年表だけが採用（approved）に進み、handoffに乗ります。
- チェックしなかった名言・機能・供給は status が `candidate` のまま**一切変わりません**。そして未処理の候補は、次にパケットを作り直せば**何度でも出てきます**。失われません。

なので、実際の進め方はこうなります:
```
# 1) まず年表だけチェックして適用
python scripts/s5_apply.py --packet data/review/RV_XXXX.md      # 反映: approved:NN（年表分だけ）
# 2) 後日、名言だけの新しいパケットを作って続きをやる
python scripts/s4_packet.py --kinds quote                       # 適用済みの年表は出てこない
# 3) 機能・供給も同様に
python scripts/s4_packet.py --kinds capability
python scripts/s4_packet.py --kinds profile
```
⚠ **一度 `s5_apply` を通したパケットファイルは再利用しないでください。** 役目を終えています。別の種別をやるときは、上のように `s4_packet` で**新しいパケットを作り直す**のが安全です（古いパケットの再適用は、処理済み分は二重にならないものの、混乱のもとです）。

反映されるもの: カード状態（verified/approved/rejected）、台帳（open→closed移動）、パケットの `補足:` 行（handoffへ転記）、
そして `data/handoff/handoff_*.txt` — **shizuku-wikiスキルの受け渡し形式そのまま**
（年表=【日付】【出来事の概要】【ソースタイトル】【ソースURL】、名言=逐語＋出典タイトル＋URL、回収成立=括弧書き追補の材料）。動画タイトルはwikiの出典表記に使うため各ブロックに含まれます。
